...
\author{von Emanuel Regnath, Martin Zellner, Alexander Preißner, Hendrik Böttcher, Samuel Harder}	
...
\begin{sectionbox}
	...
	$f_{\Y}(y) = \sum\limits_{i = 1}^{N}{f_{\X}(x_i)\left[\abs{\frac{\diff g(x)}{\diff x}(x)}_{x = x_i}\right]^{-1}}$ mit $i \in \{1,\dots,N\}$ \\
...
\begin{sectionbox}
	...
	Eine Zufallsfolge ($\mathsf{\X}_{n}: n \in \mathbb{N}$) heißt \emph{Markowkette}, falls $\forall \ n_{i} \in \N, \\
	...
	\begin{itemize}
	\item $p_{\X_{n_k}|\X_{n_{k-1}},\X_{n_{k-2}},...,\X_{n_1}}(x_{n_k}|x_{n_{k-1}},x_{n_{k-2}},...,x_{n_1})$
	$=p_{\X_{n_k}|\X_{n_{k-1}}}(x_{n_k}|x_{n_{k-1}})$ \\
	\item $f_{\X_{n_k}|\X_{n_{k-1}},\X_{n_{k-2}},...,\X_{n_1}}(x_{n_k}|x_{n_{k-1}},x_{n_{k-2}},...,x_{n_1})$
	$=f_{\X_{n_k}|\X_{n_{k-1}}}(x_{n_k}|x_{n_{k-1}})$
	\end{itemize}
	\paragraph{Zustandswahrscheinlichkeit:}
	$p_{\X_{n} | \X_{n-1}}(x_{n} | x_{n-1})$\\
	$p_{\X_1,...,\X_n}(x_1,...,x_n)=p_{\X_1}(x_1)\prod\limits_{i=2}^{n}p_{\X_i|\X_{i-1}}(x_i|x_{i-1})$
	\paragraph{Zustandsübergangsdichte:}
	$ f_{\X_{n} | \X_{n-1}}(x_{n} | x_{n-1})$\\
	${\X_1,...,\X_n}(x_1,...,x_n)=f_{\X_1}(x_1)\prod\limits_{i=2}^{n}f_{\X_i|\X_{i-1}}(x_i|x_{i-1})$ \\ \\
	...
	\emph{2-Schritt-Übergangswahrscheinlichkeit} wird wie folgt berechnet: \\
	...
	\emph{Übergangsmatrix:}
	 ...
	 \emph{Übergangswahrscheinlichkeit:} $p_{i,j} = p_{\X_{n+1}|\X_n}(x_i,x_j)$ \\

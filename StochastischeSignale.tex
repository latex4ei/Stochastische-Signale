...
\begin{sectionbox}
	...
	$\P_B(A) = \P(A|B) = \frac{\P(A \cap B)}{\P(B)} = \frac{\P(B|A) \cdot \P(A)}{\P(B)} $\\ %\qquad\quad $\P(B|A) = \P(A|B) \frac{\P(B)}{\P(A)}$\\
...
%\begin{sectionbox}
	...
	pdf: $\int_{-\infty}^\infty f_{\X}(x) \diff x \stackrel{!}{=} 1$\\ % $f_{\X}(x) \geq 0 \forall y$, 
	Für Kombinierte: $\sum p(x) + \int_{\R} f_{\X}(x) \diff x \stackrel{!}{=} 1$\\
	
	$P(\{X \in [a;b]\}) = \int_a^b f_X(x) d x$ 
\end{sectionbox}
\vspace{-0.15cm}
% --------------------------------------------------------
\begin{sectionbox}
...
\begin{sectionbox}
	\subsection{Definition}
	Der Erwartungswert ist der Schwerpunkt der Wahrscheinlichkeitsmasse und gibt den mittleren Wert einer Zufallsvariablen an
\end{sectionbox}
...
\begin{sectionbox}
	...
	\boxed{\E[\X] = \int_\mathbb R x \cdot f_{\X} (x) \diff x} für $\X : \Omega \ra \mathbb R$
	
	Für Funktionen von Zufallsvariablen:
	
	$E[\Y] = E[g(\X)] = \int \limits_{\mathbb R} g(x) f_{\X} (x) \diff x$\\
	mit $\X : \Omega \ra \mathbb R $ und $g: \mathbb R \ra \mathbb R$\\
	
	Beispiele: $E[X^2] = \int_{\mathbb{R}} x^2 f_{\X}(x) dx \\
	E[\X \Y] = \int_{\mathbb{R}} \int_{\mathbb{R}} x y f_{\X,\Y}(x,y) dx dy$ \\
\end{sectionbox}
...
\begin{sectionbox}
	...
	Kovarianzmatrix $\mathbf{C}$ von $\begin{bmatrix}
	X\\
	Y
	\end{bmatrix} $ : $\mathbf{C} = \begin{pmatrix}
	\Var[X] & \Cov[X,Y] \\
	\Cov[Y,X] & \Var[Y] \\
	\end{pmatrix}$
\end{sectionbox}
...
\begin{sectionbox}
	...
	\\
	$\Var [ \X+ \N] = Var[\X] + Var[\N] + 2 \cdot \Cov [ \X \N ]$
\end{sectionbox}
...
\begin{sectionbox}
	\subsection{Orthogonalität}
		Zwei Zufallsvariablen sind orthogonal, wenn der Korrelationswert der beiden Zufallsvariablen Null ist.
	\begin{emphbox}
		$\E[\X\Y] = 0$
	\end{emphbox}
...
\begin{sectionbox}
	...
	\begin{tablebox}{@{\extracolsep\fill}lll@{}}
		$\mu_{\X}(i) = \mu_{\X}(i + k) \qquad$ z.B. $\mu_{\X}(i)=$ const. \\
...
\begin{sectionbox}
	...
	\emph{Übergangsdiagramm:} Wie Zustandsfolgediagramm (state diagram).
...
\begin{sectionbox}
	...
	\begin{emphbox}
		\raggedright
		\textbf{Erwartungswertfunktion:} \\ $\mu_{\X} (t) = \E[\X_t]$ \\[0.5em]
...
\begin{itemize}
	...
	\item $\P(\{\W_0 = 0\}) = 1 \quad \Rightarrow \quad \P(\{\W_0 = 1\}) = 0$
